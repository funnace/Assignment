...
const EditTaskForm = ({ task, onSubmit, onClose }: { task: Task; onSubmit: () => void; onClose: () => void }) => {
  const id = task._id
  const [title, setTitle] = useState(task.title);
  const [priority, setPriority] = useState(task.priority);
  const [status, setStatus] = useState(task.status);
  const [startTime, setStartTime] = useState(new Date(task.startTime).toISOString().slice(0, 16));
  const [endTime, setEndTime] = useState(new Date(task.endTime).toISOString().slice(0, 16));
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axios.put(`${process.env.REACT_APP_API_URL}/tasks/${task._id}`, data, { headers });
      onSubmit();
    } catch (error) {
  ...
  return (
<div className="relative bg-white rounded-md shadow-lg p-8 max-w-4xl mx-auto z-10 space-y-5">
<span className='text-3xl font-bold'>Edit Task</span>
<h3>Task ID: {id}</h3>
